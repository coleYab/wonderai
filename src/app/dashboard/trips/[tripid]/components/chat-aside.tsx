  ...
  const sendMessage = async () => {
    ...
    try {
      ...
      setMessages((prev) => [
        ...
        {
          sender: 'Assistant',
          text: payload.assistant_message || 'Updated your trip details based on your request.'
        }
      ]);
    } catch (error) {
      const errorMessage =
  ...
  const confirmPendingAction = async (confirmed: boolean) => {
    ...
    try {
      ...
      setMessages((prev) => [
        ...
        {
          sender: 'Assistant',
          text: payload.assistant_message || 'Applied the confirmed update to your trip.'
        }
      ]);
      setPendingAction(null);
    } catch (error) {
        ...
        <div className='border-t border-white/10 bg-slate-950/95 p-3 pb-[max(0.75rem,env(safe-area-inset-bottom))]'>
          <div className='flex gap-2 rounded-[24px] border border-white/10 bg-white/6 p-2 shadow-inner shadow-slate-950/20'>
            <input
              ...
              placeholder={
                pendingAction
                  ? 'Confirm or cancel the pending change above'
                  : 'Ask about routes, hotels, or timing'
              }
