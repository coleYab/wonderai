...
import { readStoredTrips, updateStoredTripPlaces } from '@/features/trips/lib/custom-trips-storage';
import {
  addLoyaltyPoints,
...
export default function Home() {
  ...
  const [isMobileTripListFullscreen, setIsMobileTripListFullscreen] = useState(false);
  const [editingPlace, setEditingPlace] = useState<Place | null>(null);
  const [editingPlaceSourceName, setEditingPlaceSourceName] = useState<string | null>(null);
  ...
  const handleDeletePlace = (placeName: string) => {
    ...
    setSelectedPlace((currentPlace) => (currentPlace?.name === placeName ? null : currentPlace));
    setEditingPlaceSourceName(null);
    setEditingPlace(null);
  };
      ...
      <div className='relative min-h-full w-full overflow-hidden bg-slate-950'>
        <aside
          className={`fixed inset-x-3 bottom-3 z-10 overflow-hidden rounded-[28px] border border-white/12 bg-slate-950/78 shadow-[0_28px_80px_rgba(15,23,42,0.34)] backdrop-blur-xl transition-all duration-300 md:left-5 md:right-auto md:w-[min(420px,calc(100vw-2.5rem))] ${
            ...
            isTripListMinimized ? 'md:top-5 md:bottom-auto md:h-[176px]' : 'md:top-5 md:bottom-5'
          }`}
        >
          <div className='absolute right-2 top-2 z-20 flex items-center gap-1.5'>
            <button
              ...
              aria-label={
                isTripListMinimized ? 'Maximize trip plan list' : 'Minimize trip plan list'
              }
            ...
            <button
              ...
              aria-label={
                isMobileTripListFullscreen ? 'Shrink trip plan list' : 'Expand trip plan list'
              }
            ...
            >
              {isMobileTripListFullscreen ? (
                <svg
                  ...
                  aria-hidden='true'
                >
                  <path strokeLinecap='round' strokeLinejoin='round' d='M9 9H4v5' />
                  <path strokeLinecap='round' strokeLinejoin='round' d='m4 14 6-6' />
                  <path strokeLinecap='round' strokeLinejoin='round' d='M15 15h5v-5' />
                  <path strokeLinecap='round' strokeLinejoin='round' d='m20 10-6 6' />
                </svg>
              ) : (
                <svg
                  xmlns='http://www.w3.org/2000/svg'
                  ...
                  aria-hidden='true'
                >
                  <path strokeLinecap='round' strokeLinejoin='round' d='M9 4H4v5' />
                  <path strokeLinecap='round' strokeLinejoin='round' d='M15 4h5v5' />
                  <path strokeLinecap='round' strokeLinejoin='round' d='M4 15v5h5' />
                  <path strokeLinecap='round' strokeLinejoin='round' d='M20 15v5h-5' />
                </svg>
              )}
            </button>
          </div>
          ...
          <DialogContent className='sm:max-w-md'>
            ...
            <div className='rounded-lg border border-slate-200 bg-slate-50 px-4 py-3 text-sm text-slate-700'>
              {isCalendarStatusLoading ? (
                ...
                <span>
                  {places.length} event{places.length === 1 ? '' : 's'} will be created from this
                  trip.
                </span>
              ) : (
        ...
        <BotIcon isChatOpen={isChatOpen} onToggle={() => setIsChatOpen((prev) => !prev)} />
        ...
        {editingPlace ? (
          <div
            className='fixed inset-0 z-40 flex items-start justify-center overflow-y-auto bg-slate-900/20 px-4 py-4 backdrop-blur-[2px]'
            onClick={(event) => {
          ...
          >
            <div className='my-2 w-full max-w-3xl overflow-y-auto rounded-[18px] border border-slate-200 bg-white text-slate-900 shadow-[0_24px_80px_rgba(15,23,42,0.18)] sm:my-6 sm:max-h-[calc(100dvh-3rem)]'>
              <div className='flex items-start justify-between gap-4 border-b border-slate-200 px-5 py-4'>
                <div>
                  ...
                  <p className='mt-1 text-sm text-slate-600'>
                    Search from the place list, then adjust day, date, and time range.
                  </p>
                </div>
                <button
                  type='button'
                ...
                <section className='space-y-4 rounded-[16px] border border-slate-200 bg-white p-4'>
                  ...
                  <div className='rounded-md border border-slate-200 bg-slate-50 p-4'>
                    ...
                    <p className='mt-3 text-sm text-slate-600'>
                      Use the schedule controls below. The place itself comes from the search list.
                    </p>
                  </div>
                  ...
                  <div className='flex flex-wrap gap-3 pt-1'>
                    ...
                    <button
                      type='button'
                      disabled={completedPlaceNames.includes(editingPlace.name)}
                      onClick={() => handleDeletePlace(editingPlaceSourceName ?? editingPlace.name)}
